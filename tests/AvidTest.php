...
User.fill([{"name":"foo","email":"bar@foo.com"}]);
EOT
            , $ardent->script()
        );
    }
    ...
    public function testItCanAddCollectionToStack()
    {
        ...
        $this->assertEquals(<<<EOT
User.fill([{"name":"foo","email":"bar@foo.com"},{"name":"foo2","email":"bar2@foo.com"}]);
EOT
            , $ardent->script()
        );
    }
    ...
    public function testModelNameIsUsedInJavaScriptCode()
    {
        ...
        $this->assertContains('Home.fill(', $jsCode);
    }
    ...
    public function testItCanHandleMultipleModelTypes()
    {
        ...
        $this->assertEquals(<<<EOT
User.fill([{"name":"foo","email":"bar@foo.com"},{"name":"foo2","email":"bar2@foo.com"}]);
Home.fill([{"id":1,"name":"unknown","user_id":1}]);
EOT
            , $ardent->script());
    }
    ...
    public function testItCanHandleRelations()
    {
        ...
        $ardent->add($home);
        $this->assertEquals(<<<EOT
Home.fill([{"id":1,"name":"unknown","user_id":1,"user":{"id":1,"name":"foo","email":"bar@foo.com"}}]);
EOT
            , $ardent->script());
    }
